...
let animeFinished=!0
async function setIndex(index){indexImg=index
...
async function addActiveClass(){animeFinished=!1
...
await new Promise(resolve=>{setTimeout(()=>{tabImg[nextImg].classList.remove("active");tabImg[prevImg].classList.remove("active");animeFinished=!0;resolve()},600)})}
addActiveClass()
nextBtnCaroussel.addEventListener("click",function(){if(!animeFinished)return
async function slideNext(){await setIndex(nextImg)
addActiveClass()}
slideNext()})
prevBtnCaroussel.addEventListener("click",function(){if(!animeFinished)return
async function slidePrev(){await setIndex(prevImg)
addActiveClass()}
slidePrev()})
indicators[0].addEventListener("click",function(e){if(e.target&&e.target.nodeName==="BUTTON"){if(!animeFinished)return
Array.from(indicators[0].children).forEach(indicator=>{indicator.classList.remove("active")
...
addActiveClass()}})
let autoSlide=setInterval(function(){if(!animeFinished)return
async function slideNext(){await setIndex(nextImg)
addActiveClass()}
slideNext()},5000)
const carousel=document.querySelector(".carousel")
carousel.addEventListener("mouseover",function(){clearInterval(autoSlide)})
carousel.addEventListener("mouseout",function(){autoSlide=setInterval(function(){async function slideNext(){await setIndex(nextImg)
addActiveClass()}
slideNext()},5000)})